function func_concatProc2binary(p)
...
tab = readtable([p.procDatapn p.RHDfolderpn 'parsedMatfiles\parsedMatfilesMetadata.xlsx']);
...
binFilename = [p.sessionpn(1:end-1) '_phase3' '_128spkContinuous'];


disp('Concatenating parsed multichannel data and writing large single binary file...')
fileID = fopen([p.procDatapn p.RHDfolderpn binFilename '.bin'], 'wb');
...
for iRHDfile = 1:nRHDfiles
    ...
    for iChan = 1:nChans
        load([p.procParsedPn tabiRHD.parsedFilename{iChan}]); % loads variable called "data"
        dataJoin(iChan,:) = data;
