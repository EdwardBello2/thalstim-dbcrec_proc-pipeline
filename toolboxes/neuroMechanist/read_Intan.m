function intan_data = read_Intan(varargin)
%
...
%
% OUTPUTS
%   intan_info structure with all the default Intan variables as fields
%
%
% revised by Daniel Leventhal, 12-06-2019 to accept a filename as an input
% and spit out an intan_data structure as output
%
% updated by Seyed Yahya Shirazi, 04-16-2023 to update to
% read_intan_RHD2000_file v 3.0 and remove unnecessary prompts.
...
% DL, 12/2019: Instead of moving variables to base workspace, place them in
% a data structure that can be output from this function

intan_data.filename = file;
intan_data.path = path;

intan_data.notes = notes;

intan_data.frequency_parameters = frequency_parameters;
if (data_file_main_version_number > 1)
    intan_data.reference_channel = reference_channel;
end

if (num_amplifier_channels > 0)
    intan_data.amplifier_channels = amplifier_channels;
    if (data_present)
        intan_data.amplifier_data = amplifier_data;
        intan_data.t_amplifier = t_amplifier;
    end
    intan_data.spike_triggers = spike_triggers;
end
if (num_aux_input_channels > 0)
    intan_data.aux_input_channels = aux_input_channels;
    if (data_present)
        intan_data.aux_input_data = aux_input_data;
        intan_data.t_aux_input = t_aux_input;
    end
end
if (num_supply_voltage_channels > 0)
    intan_data.supply_voltage_channels = supply_voltage_channels;
    if (data_present)
        intan_data.supply_voltage_data = supply_voltage_data;
        intan_data.t_supply_voltage = t_supply_voltage;
    end
end
if (num_board_adc_channels > 0)
    intan_data.board_adc_channels = board_adc_channels;
    if (data_present)
        intan_data.board_adc_data = board_adc_data;
        intan_data.t_board_adc = t_board_adc;
    end
end
if (num_board_dig_in_channels > 0)
    intan_data.board_dig_in_channels = board_dig_in_channels;
    if (data_present)
        intan_data.board_dig_in_data = board_dig_in_data;
        intan_data.t_dig = t_dig;
    end
end
if (num_board_dig_out_channels > 0)
    intan_data.board_dig_out_channels = board_dig_out_channels;
    if (data_present)
        intan_data.board_dig_out_data = board_dig_out_data;
        intan_data.t_dig = t_dig;
    end
end
if (num_temp_sensor_channels > 0)
    if (data_present)
        intan_data.temp_sensor_data = temp_sensor_data;
        intan_data.t_temp_sensor = t_temp_sensor;
    end
end

fprintf(1, 'Done!  Elapsed time: %0.1f seconds\n', toc);
if (data_present)
    fprintf(1, 'Data successfully is exported.\n');
else
    fprintf(1, 'Only waveform information is exported.\n');
end
fprintf(1, '\n');
